...
dim UserID, LanguageID, SceneID, LoginName, Password, PswQuestion, PswAnswer, UserStatus, RegTime, RegIP, LastLoginIP, LastLoginTime, LoginCount, TrueName, Gender, Birthday, Country, Province, City, Address, PostCode, Phone, Mobile, Email, QQ, MSN, Permissions, AdminDesc,adminlanmu
...
Sub addAdmin	
	...
	LoginCount=0
	Gender=1
	adminlanmu=replace(getForm("adminlanmu","post")," ","")
	...
	if isnul(Session("adminlanmu")) then '增加判断，如果选择了栏目权限 则不允许添加管理员 无添加权限
    sql="insert into {prefix}User( GroupID, LoginName, [Password], UserStatus, RegTime, RegIP, LoginCount, Gender, AdminDesc,adminlanmu) values("&GroupID&", '"&LoginName&"', '"&md5(Password,16)&"', "&UserStatus&", '"&RegTime&"', '"&RegIP&"', "&LoginCount&", "&Gender&", '"&AdminDesc&"','"&adminlanmu&"')"
    end if
...
Sub editAdmin
	...
	if isnul(Session("adminlanmu")) then

    passStr=passStr&" , [adminlanmu]='"&adminlanmu&"'"
