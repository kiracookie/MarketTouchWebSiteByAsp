...
Function makeOption(tableName,fieldText,fieldValue,selected,strOrder,ParentID)
	Dim rs ,sel
	dim adminlanmu,lanmusql
    adminlanmu=Session("adminlanmu")
    if not isnul(adminlanmu) then
    lanmusql=" and instr(',"&adminlanmu&",',','+CStr(SortID)+',')>0 "
    end if
    sel=""
    set rs =conn.Exec ("select ["&fieldValue&"],["&fieldText&"],ParentID,SortLevel,(select count(*) from {prefix}Sort where ParentID=t.SortID) as c from "&tableName&" as t where LanguageID="&Session("languageID")&" and ParentID="&ParentID&lanmusql&" "&strOrder,"r1")
	Do While Not rs.Eof
