...
'解析导航栏
   Public Function parseNavList(str)
       ...
       dim vtype,vnum,whereStr,linkArray,star
       ...
       for each match in matches
           ...
           topNum=cint(parseArr(labelAttrLinklist)("num"))
           if not isNul(topNum) and topNum>0 then topStr="top "&topNum
               '读取参数star
             star=parseArr(labelAttrLinklist)("star")
             if isnul(star) then
              star=""
             else
             if isnum(star) then
              star=star-1
              star="sortID NOT IN (SELECT TOP "&star&" sortID from {prefix}Sort where LanguageID="&setting.languageID&" and SortStatus=1 and ParentID="&vtype&" order by SortOrder asc) and "
             else
              star=""
             end if
             end if

           linkArray=conn.Exec("select "&topStr&" SortName,SortType,SortURL,sortID,(select count (*) from {prefix}Sort as a where a.ParentID=b.sortID) as subcount,SortFolder,SortFileName,GroupID,Exclusive,indeximage,SortEnName,IcoImage,PageDesc,SortContent,PageTitle from {prefix}Sort as b  where "&star&" LanguageID="&setting.languageID&" and SortStatus=1 and ParentID="&vtype&" order by SortOrder asc","arr")
           ...
           for i=0 to vnum
               ...
               for each matchfield in matchesfield
                   ...
                   set contentNum=conn.Exec("select count(*) from AspCms_Content where (sortID in(select sortid from AspCms_Sort where (ParentID = "&linkArray(3,i)&" or sortID = "&linkArray(3,i)&") and ContentStatus<>0))","r1")
                   ...
                   select case fieldName
                       ...
                       case "pagetitle"
                           loopstrLinklistNew=replaceStr(loopstrLinklistNew,matchfield.value,linkArray(14,i))
                       case "desc"
	...
	Public Function parseLoop(Byval str)
		...
		for each match in matches
			...
			elseif str="about" or str="type" then			
				sql="select SortType,SortID,SortURL,SortFolder,SortFileName,SortName,SortContent,GroupID,ImagePath,IcoImage,IndexImage,Exclusive,PageDesc,SortEnName,PageKeywords,PageTitle from {prefix}Sort where SortStatus=1 and SortID="&lsort&""
			    ...
			    for each matchfield in matchesfield
						...
						select case fieldName
							...
							case "date"
								...
								select case timestyle
									...
									case "y"
										nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj("AddTime"),12))
								end select
							case "visits"
                            ...
                            '增加获取缩略图功能 2017年4月24日21:56:04 by KiraCookie
                            case "imagePath"
                                if not isNul(rsObj("ImagePath")) then
                                    if instr(rsObj("ImagePath"),"http://")>0 then
                                        nloopstr = replace(nloopstr,matchfield.value,rsObj("ImagePath"))
                                    else
                                        nloopstr = replace(nloopstr,matchfield.value,rsObj("ImagePath"))
                                    end if
                                else
					...
					elseif str="type" then
						select case fieldName
                            ...
                            '增加页面标题 2017年4月24日21:56:04 by KiraCookie
                            case "pageTitle"
                                if not isNul(rsObj("ImagePath")) then
                                    if instr(rsObj("ImagePath"),"http://")>0 then
                                        nloopstr = replace(nloopstr,matchfield.value,rsObj("PageTitle"))
                                    else
                                        nloopstr = replace(nloopstr,matchfield.value,rsObj("PageTitle"))
                                    end if
                                else
							...
							case "wdate"
								timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
								 select case timestyle
									...
									case "y"
										nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(6,i),12))
								end select	
							case "rdate"
								timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
								 select case timestyle
									...
									case "y"
										nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(7,i),12))
								end select	
						end select	
					elseif str="comment" then
						select case fieldName		
								...
								case "date"
									timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
									 select case timestyle
										...
										case "m"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(4,i),9))
										case "d"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(4,i),10))
										case "yy"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(4,i),11))
										case "y"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(DateArray(4,i),12))
									end select	
							end select		
								...
								case "wdate"
									timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
									 select case timestyle
										...
										case "y"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(6),12))
									end select	
								case "rdate"
									timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
									 select case timestyle
										...
										case "y"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(7),12))
									end select	
							end select
						elseif pageListType="userbuylist" then
								...
								case "OrderTime"									
									timestyle = parseArr(fieldArr)("style") : if isNul(timestyle) then timestyle = "m-d"
									 select case timestyle
										...
										case "m"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(3),9))
										case "d"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(3),10))
										case "yy"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(3),11))
										case "y"
											nloopstr = replace(nloopstr,matchfield.value,FormatDate(rsObj(3),12))
										end select	
								case "State"
									dim statecn
