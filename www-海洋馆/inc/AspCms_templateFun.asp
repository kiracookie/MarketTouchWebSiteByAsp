...
Function makeAbout(Byval sortID, Byval page, Byval isMakeHtml)
	...
	templatePath=sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/"	&templateFile
	...
	if isExistStr(content,"{aspcms:page}") then		
		...
		htmlfilename=ewap&"?"&sortID
		if runMode=1 then htmlfilename=replace(rsObj("SortFolder"), "{sitepath}", sitePath&swap)&replace(rsObj("SortFileName"), "{sortid}", sortID)
		htmlPath=replace(rsObj("SortFolder"), "{sitepath}", sitePath&swap)
...
Function makeUser(Byval userid, Byval page, Byval isMakeHtml)
	...
	templatePath=sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/userinfo.html"
	'die templatePath
...
Function struserbuy(Byval userid, Byval page)
	...
	templatePath=sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/userbuy.html"
	'die templatePath
	if not CheckTemplateFile(templatePath) then echo templateFile&err_16 : exit function
...
Function makeList(Byval sortID)
	...
	for page=1 to pcount	
		...
		sortFolder=rsObj("sortFolder")
		sortFileName=rsObj("sortFileName")		
		sortFolder=replace(sortFolder, "{sitepath}", sitePath&swap)	
		sortFileName=replace(sortFileName, "{sortid}", sortID)			
