...
Class TemplateClass
	...
	Public Function parseTopAndFoot()
		...
		if isExistStr(content,"{aspcms:top}") then content=replaceStr(content,"{aspcms:top}",loadFile(sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/head.html"))
		if isExistStr(content,"{aspcms:foot}") then content=replaceStr(content,"{aspcms:foot}",sharestr&"<script type=""text/javascript"" src="""&sitePath&"/"&"inc/AspCms_Statistics.asp""></script>"&loadFile(sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/foot.html"))	
	End Function
	...
	Public Function parseAuxiliaryTemplate()
		...
		for each matchRuxiliary in matchesRuxiliary			
			...
			content=replaceStr(content,"{aspcms:template src="&srcTemplate&"}",loadFile(sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&twap&"/"&srcTemplate))
		next
	...
	Public Function parseGlobal()		
		...
		dim cnzzstr:cnzzstr="":	if not isnul(CNZZUSER) then cnzzstr="<div style='display:none;'><script src='http://pw.cnzz.com/c.php?id="&split(CNZZUSER,"@")(0)&"&l=2' language='JavaScript' charset='gb2312'></script></div>"
		...
		if runMode=1 then
		content=replaceStr(content,"{aspcms:homeurl}",swap&"/"&"index"&fileExt)
		else
		content=replaceStr(content,"{aspcms:homeurl}",swap&"/"&"?index"&fileExt)
		end if
	...
	Public Function parseNavList(str)	
		...
		dim vtype,vnum,whereStr,linkArray,vwap,vnav,vnav2,vwapnav,vwapnav2,wapstr,navstr,navstr2,wapnavstr,wapnavstr2
		...
		for each match in matches
			...
			vwap=parseArr(labelAttrLinklist)("wap")
			if isnul(vwap) then 
			    wapstr=""
			else
			    wapstr="and WapStatus=1 "
			end if
			vnav=parseArr(labelAttrLinklist)("nav")
			if isnul(vnav) then 
			    navstr=""
			else
			    navstr="and NavStatus=1 "
			end if
			vnav2=parseArr(labelAttrLinklist)("nav2")
			if isnul(vnav2) then 
			    navstr2=""
			else
			    navstr2="and NavStatus2=1 "
			end if
			vwapnav=parseArr(labelAttrLinklist)("wapnav")
			if isnul(vwapnav) then 
			    wapnavstr=""
			else
			    wapnavstr="and WapNavStatus=1 "
			end if
			vwapnav2=parseArr(labelAttrLinklist)("wapnav2")
			if isnul(vwapnav2) then 
			    wapnavstr2=""
			else
			    wapnavstr2="and WapNavStatus2=1 "
			end if
				
			linkArray=conn.Exec("select SortName,SortType,SortURL,sortID,(select count (*) from {prefix}Sort as a where a.ParentID=b.sortID) as subcount,SortFolder,SortFileName,GroupID,Exclusive,indeximage,SortEnName,IcoImage,PageDesc,SortContent from {prefix}Sort as b  where LanguageID="&setting.languageID&" and SortStatus=1 "&wapstr&navstr&navstr2&wapnavstr&wapnavstr2&"and ParentID="&vtype&" order by SortOrder asc","arr")
...
case "ico"
	...
	Function getSortLink(sortType, sortID, sortUrl, sortFolder, sortFileName, GroupID, Exclusive)	
		sortFolder=replace(repnull(sortFolder), "{sitepath}", sitePath&swap)	
		...
		if sortType="7" then 
			...
			if runMode=1 and viewNoRight(GroupID, Exclusive) then
				...
				Select  case sortType					
					case "1" 			
						getSortLink=sitePath&setting.languagePath&ewap&"about"&"/?"&sortID&fileExt
					case else
						getSortLink=sitePath&setting.languagePath&ewap&"list"&"/?"&sortID&"_1"&fileExt											
				End Select
			end if
		end if
	End Function
	...
	Function getContentLink(Byval SortID,Byval Id,Byval SortFolder,Byval GroupID,Byval ContentFolder,Byval ContentFileName,Byval ContentTime,Byval pageFileName,Byval SortGroupID)		
		ContentFolder=replace(ContentFolder, "{sitepath}", sitePath&swap)	
		...
		if runMode=1 and not isnul(SortFolder) then 
			if GroupID>2 or SortGroupID>2 then 
				getContentLink=sitePath&setting.languagePath&ewap&"content/"&"?"&Id&fileExt
