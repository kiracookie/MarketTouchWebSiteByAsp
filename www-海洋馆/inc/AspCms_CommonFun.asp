...
Function ClearHtml(HtmlContent)
       if GHtml=1 then
		HtmlContent=ReplaceHtml("\r\n", "", HtmlContent) 
		HtmlContent=ReplaceHtml("\n", "", HtmlContent)
		HtmlContent=ReplaceHtml("\t", "", HtmlContent)
		HtmlContent=ReplaceHtml(">[\s]+<", "><", HtmlContent)
		HtmlContent=ReplaceHtml("<!--[^!]*-->", "", HtmlContent)
		'HtmlContent=ReplaceHtml("\" ", """, HtmlContent)
		'HtmlContent=ReplaceHtml(" \"", """, HtmlContent)
		HtmlContent=ReplaceHtml("/\*[^*]*\*/", "", HtmlContent)
		end if
		ClearHtml=HtmlContent
End Function

Function dropHtml2str(strContent)
        strContent=ClearHtml(strContent)
		strContent=ReplaceHtml("<[a-zA-Z].*?>", "", strContent)
		strContent=ReplaceHtml("<[\/][a-zA-Z].*?>", "", strContent)		
		dropHtml2str=strContent
End Function
...
Function dropHtml(Byval strHTML) 
	...
	'取闭合的<> 
	objRegExp.Pattern = "<.+?>" 
	if isnul(strHTML) or strHTML="" then
	strHTML="<p>&nbsp;</p>"
	end if
	'进行匹配 
