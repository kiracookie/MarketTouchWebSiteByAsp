...
Select case action	
	...
	case "onnav" : onOff "on", "Sort", "SortID", "NavStatus", "", getPageName()
	case "offnav" : onOff "off", "Sort", "SortID", "NavStatus", "", getPageName()
	case "onnav2" : onOff "on", "Sort", "SortID", "NavStatus2", "", getPageName()
	case "offnav2" : onOff "off", "Sort", "SortID", "NavStatus2", "", getPageName()
	case "onwap" : onOff "on", "Sort", "SortID", "WapStatus", "", getPageName()
	case "offwap" : onOff "off", "Sort", "SortID", "WapStatus", "", getPageName()
	case "onwapnav" : onOff "on", "Sort", "SortID", "WapNavStatus", "", getPageName()
	case "offwapnav" : onOff "off", "Sort", "SortID", "WapNavStatus", "", getPageName()
	case "onwapnav2" : onOff "on", "Sort", "SortID", "WapNavStatus2", "", getPageName()
	case "offwapnav2" : onOff "off", "Sort", "SortID", "WapNavStatus2", "", getPageName()
	case "move" : moveSort
End Select

dim SortID, LanguageID, ParentID, SortOrder, SortType, SortName, SortURL, SortLevel, AddTime, PageTitle, PageKeywords, PageDesc, SortPath, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, WapStatus, NavStatus, NavStatus2, WapNavStatus, WapNavStatus2, TopSortID, GroupID, Exclusive, Content,indeximage,IcoPath,IcoImage
...
Sub getSort
	...
	if not isnul(ID) then		
		...
		if rs.eof then 
			...
			NavStatus=rs("NavStatus")
			NavStatus2=rs("NavStatus2")
			WapStatus=rs("WapStatus")
			WapNavStatus=rs("WapNavStatus")
			WapNavStatus2=rs("WapNavStatus2")
...
Sub quickAdd
	...
	for i=1 to 10	
		...
		if not isnul(TopSortName) then 
			...
			conn.exec "insert into {prefix}Sort(ParentID, SortOrder, SortType, SortName, SortURL, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, NavStatus, NavStatus2, WapStatus, WapNavStatus, WapNavStatus2, LanguageID, AddTime, GroupID, Exclusive,IcoPath,IcoImage) values("&ParentID&", "&SortOrder&", "&SortType&", '"&TopSortName&"', '"&SortURL&"', '"&SortTemplate&"', '"&ContentTemplate&"', '"&SortFolder&"', '"&ContentFolder&"', '"&SortFileName&"', '"&ContentFileName&"', "&SortStatus&", "&NavStatus&", "&NavStatus2&", "&WapStatus&", "&WapNavStatus&", "&WapNavStatus2&", "&LanguageID&", '"&AddTime&"', "&GroupID&", '"&Exclusive&"','"&IcoPath&"','"&IcoImage&"')", "exe"
			...
			if not isnul(SubSortName) then
				...
				for j=0 to ubound(SubSortName)	
					if not isnul(SubSortName(j)) then
						conn.exec "insert into {prefix}Sort(ParentID, SortOrder, SortType, SortName, SortURL, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, NavStatus, NavStatus2, WapStatus, WapNavStatus, WapNavStatus2, LanguageID, AddTime, GroupID, Exclusive) values("&ParentID&", "&SortOrder&", "&SortType&", '"&SubSortName(j)&"', '"&SortURL&"', '"&SortTemplate&"', '"&ContentTemplate&"', '"&SortFolder&"', '"&ContentFolder&"', '"&SortFileName&"', '"&ContentFileName&"', "&SortStatus&", "&NavStatus&", "&NavStatus2&", "&WapStatus&", "&WapNavStatus&", "&WapNavStatus2&", "&LanguageID&", '"&AddTime&"', "&GroupID&", '"&Exclusive&"')", "exe"
					end if
				next
			end if		
			editSubSort(sortID)	
...
function sortList(ParentID)
	Dim rs :set rs =Conn.Exec ("select *,(select count(*) from {prefix}Sort where ParentID=t.SortID) as c from {prefix}Sort t where LanguageID="&session("languageID")&" and ParentID="&ParentID&" order by Sortorder,Sortorder ","r1")
	
	dim sortTypenames : sortTypenames=split(sortTypes,",")
	dim showtype
	
	IF rs.eof or rs.bof Then
		echo "<td colspan=""13"">还没有记录</td></tr>"
	Else
		Do While not rs.eof 
			showtype=""
			if rs("c")>0 then 
			...
			echo "<td height=20><input type=""checkbox"" name=""id"" value="""&rs("SortID")&""" class=""checkbox"" /><input type=""hidden"" name=""SortIDs"" value="""&rs("SortID")&""" /></td>"&vbcrlf
			echo "<td>"&rs("SortID")&"</td>"&vbcrlf
			
			echo "<td align=""left"">"&getLevel(rs("SortLevel"))&showtype&"<input name=""SortNames"" type=""text"" class=""input"" id=""SortNames"" value="""&rs("SortName")&""" maxlength=""200"" style=""width:80px;""/>"
			...
			echo "<TD align=center>"&getStr(rs("NavStatus"),"<a href=""?action=offnav&id="&rs("SortID")&""" title=""启用"" ><IMG src=""../../images/toolbar_ok.gif""></a>","<a href=""?action=onnav&id="&rs("SortID")&""" title=""禁用"" ><IMG src=""../../images/toolbar_no.gif""></a>")&"</TD>"&vbcrlf
			echo "<TD align=center>"&getStr(rs("NavStatus2"),"<a href=""?action=offnav2&id="&rs("SortID")&""" title=""启用"" ><IMG src=""../../images/toolbar_ok.gif""></a>","<a href=""?action=onnav2&id="&rs("SortID")&""" title=""禁用"" ><IMG src=""../../images/toolbar_no.gif""></a>")&"</TD>"&vbcrlf
			if wapMode=1 then     
			echo "<TD align=center>"&getStr(rs("WapStatus"),"<a href=""?action=offwap&id="&rs("SortID")&""" title=""启用"" ><IMG src=""../../images/toolbar_ok.gif""></a>","<a href=""?action=onwap&id="&rs("SortID")&""" title=""禁用"" ><IMG src=""../../images/toolbar_no.gif""></a>")&"</TD>"&vbcrlf
			echo "<TD align=center>"&getStr(rs("WapNavStatus"),"<a href=""?action=offwapnav&id="&rs("SortID")&""" title=""启用"" ><IMG src=""../../images/toolbar_ok.gif""></a>","<a href=""?action=onwapnav&id="&rs("SortID")&""" title=""禁用"" ><IMG src=""../../images/toolbar_no.gif""></a>")&"</TD>"&vbcrlf
			echo "<TD align=center>"&getStr(rs("WapNavStatus2"),"<a href=""?action=offwapnav2&id="&rs("SortID")&""" title=""启用"" ><IMG src=""../../images/toolbar_ok.gif""></a>","<a href=""?action=onwapnav2&id="&rs("SortID")&""" title=""禁用"" ><IMG src=""../../images/toolbar_no.gif""></a>")&"</TD>"&vbcrlf
			end if
...
Sub addSort	
	...
	NavStatus=getCheck(getForm("NavStatus", "post"))
	NavStatus2=getCheck(getForm("NavStatus2", "post"))
	WapStatus=getCheck(getForm("WapStatus", "post"))
	WapNavStatus=getCheck(getForm("WapNavStatus", "post"))
	WapNavStatus2=getCheck(getForm("WapNavStatus2", "post"))
	...
	conn.exec "insert into {prefix}Sort(ParentID, SortOrder, SortType, SortName,SortEnName, SortURL, PageTitle, PageKeywords, PageDesc, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, NavStatus, NavStatus2, WapStatus, WapNavStatus, WapNavStatus2, SortContent, LanguageID, AddTime, GroupID, Exclusive,indeximage,IcoImage,IcoPath) values("&ParentID&", "&SortOrder&", "&SortType&", '"&SortName&"','"&SortEnName&"', '"&SortURL&"', '"&PageTitle&"', '"&PageKeywords&"', '"&PageDesc&"', '"&SortTemplate&"', '"&ContentTemplate&"', '"&SortFolder&"', '"&ContentFolder&"', '"&SortFileName&"', '"&ContentFileName&"', "&SortStatus&", "&NavStatus&", "&NavStatus2&", "&WapStatus&", "&WapNavStatus&", "&WapNavStatus2&", '"&Content&"', "&LanguageID&", '"&AddTime&"', "&GroupID&", '"&Exclusive&"','"&indeximage&"','"&IcoImage&"','"&IcoPath&"')", "exe" 	
	SortID=Conn.Exec("select @@identity","r1")(0)
	if ParentID="0" then 
		SortLevel="1"
		TopSortID=SortID
