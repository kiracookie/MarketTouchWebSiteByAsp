...
<DIV class=formzone>
<DIV class=namezone><%if actType="html" then%>主站生成静态<%elseif actType="wap" then %>wap站生成静态<%elseif actType="map" then %>地图生成静态<%elseif actType="rss" then%>Rss生成静态<%else%>生成静态<%end if%></DIV>
<DIV class=tablezone>
...
<TR>						
        <TD align=middle width=100 height=30>按日期更新</TD>
        <TD><input maxlength="20" style="WIDTH: 100px" name="wEditTime" class="Wdate" onClick="WdatePicker()" value="<%=date()%>"/><input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=daywap'"/>  <img src="../images/help.gif" class="tTip" title="生成选中日期更新的内容"></TD>
    </TR>
    <TR>
        <TD align=middle width=100 height=30>一键生成</TD>
        <TD><input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=allwap'"/> <img src="../images/help.gif" class="tTip" title="一键将全站信息生成为静态"></TD>
    </tr>
    <TR>
        <TD align=middle width=100 height=30>生成首页</TD>
        <TD><input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=wapindex'"/> </TD></TR>
    <TR>
    <TR>
        <TD align=middle height=30>生成列表页</TD>
        <TD>
		<%LoadSelect "wsortid","Aspcms_Sort","SortName","SortID",getForm("id","get"), 0," and SortStatus=1 and SortType<>1 and SortType<>7 order by SortOrder","所有分类"%>
        <input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=waplist'"/>
        <input type="submit" class="button" value="生成所有" onClick="form.action='?actType=<%=actType%>&action=allwaplist'"/>  <img src="../images/help.gif" class="tTip" title="按照栏目生成该栏目的信息列表页">
        </TD>
    </TR>
    <TR>
        <TD align=middle height=30>生成内容页</TD>
        <TD>
		<%LoadSelect "wcsortid","Aspcms_Sort","SortName","SortID",getForm("id","get"), 0," and SortStatus=1 and SortType<>1 and SortType<>7 order by SortOrder","所有分类"%>
        <input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=wapcontent'"/>
        <input type="submit" class="button" value="生成所有" onClick="form.action='?actType=<%=actType%>&action=allwapcontent'"/> <img src="../images/help.gif" class="tTip" title="按照栏目生成该栏目的下的内容详细页">
        </TD>
    </TR>
    <TR>
        <TD align=middle height=30>生成单页</TD>
        <TD><input type="submit" class="button" value="生成" onClick="form.action='?actType=<%=actType%>&action=wapabout'"/> <img src="../images/help.gif" class="tTip" title="生成所有单独的信息页面"></TD>
    </TR>
<%    
