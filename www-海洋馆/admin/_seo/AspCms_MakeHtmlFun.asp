...
select case action
	...
	case "allwap" : checkRunMode : makeAllWap
	case "index" : checkRunMode : makeIndex
	case "wapindex" : checkRunMode : makeWapIndex
	case "about" : checkRunMode : makeAllAbout
	case "wapabout" : checkRunMode : makeAllWapAbout
	case "alllist" : checkRunMode : makeAllList
	case "allwaplist" : checkRunMode : makeAllWapList
	case "allcontent" : checkRunMode : makeAllContent
	case "allwapcontent" : checkRunMode : makeAllWapContent
	case "list" : checkRunMode : makeListBySort
	case "waplist" : checkRunMode : makeWapListBySort
	case "content" : checkRunMode : makeContentBySortID
	case "wapcontent" : checkRunMode : makeWapContentBySortID	
...
Sub makeAllWap()
	...
	makeAllWapList()
	makeAllWapContent()
	alertMsgAndGo "生成WAP全站成功","AspCms_MakeHtml.asp?actType="&actType	
End Sub
...
Sub makeByDayWap
	...
	ewap=wap&"/"
	swap="/"&wap
	EditTime=getForm("wEditTime", "post")
	if not isdate(EditTime) then alertMsgAndGo "请输入正确的日期","-1"	
	sql="select * from {prefix}Content where ContentStatus=1 and LanguageID="&cint(session("languageID"))&" and DateDiff('d', EditTime, '"&EditTime&"')<=0 order by ContentID"
	set rs=conn.Exec(sql,"r1")	
	Do While not rs.Eof
	...
	sql="select SortPath from {prefix}Sort where WapStatus=1 and SortID in(select SortID from {prefix}Content where ContentStatus=1 and LanguageID="&cint(session("languageID"))&" and DateDiff('d', EditTime, '"&EditTime&"')<=0 group by SortID)"	
	set rs=conn.Exec(sql,"r1")	
	Do While not rs.Eof
		SortIDs=SortIDs&rs(0)
		rs.moveNext
	Loop
	if not isnul(SortIDs) then 
		sql="select SortID from {prefix}Sort where WapStatus=1 and SortID in("&SortIDs&")"
		set rs=conn.Exec(sql,"r1")		
		Do While not rs.Eof	
			makeList( rs(0))
			rs.moveNext
...
Sub makeAllWapList()
	...
	swap="/"&wap
	dim rs,sql
	sql = "select SortID,SortType,SortName from {prefix}Sort where WapStatus=1 and SortType<>1 and SortType<>7 and GroupID<3 and LanguageID="&cint(session("languageID"))&" order by SortID"
	set rs=conn.Exec(sql,"r1")	
	Do While not rs.Eof
		makeList( rs("SortID"))
		'echo "生成"""&rs("SortID")&rs("SortName")&"""成功<br>"
	...
	Loop
	rs.close : set rs =nothing		
	if action<>"allwap" then alertMsgAndGo "生成列表页成功","AspCms_MakeHtml.asp?actType="&actType	
End Sub
...
'按条件生成列表页
Sub makeWapListBySort()
    wap="wap"
	twap=wap
	ewap=wap&"/"
	swap="/"&wap
	dim rs,sql,SortID
	SortID=getForm("wsortid","post")
	if SortID=0 then alertMsgAndGo "请选择栏目","MakeHtml.asp?actType="&actType	
	if not isnum(SortID) then alertMsgAndGo "分类ID不正确","MakeHtml.asp?actType="&actType	
	dim sortids	
	sortids=getSubSort(SortID, 1)
	sql="select SortID,SortType,SortName from {prefix}Sort where WapStatus=1 and SortType<>1 and SortType<>7 and GroupID<3 and LanguageID="&cint(session("languageID"))&" and SortID in ("&sortids&") order by SortID"
	set rs=conn.Exec(sql,"r1")
	Do While not rs.Eof
		makeList( rs("SortID"))
		'echo "生成"""&rs("SortName")&"""成功<br>"
...
Sub makeWapIndex
	...
	swap="/"&wap
	dim templateobj,templatePath : set templateobj = new TemplateClass	
	templatePath=sitePath&"/templates/"&setting.defaultTemplate&"/wap/index.html"	
	'die templatePath
	if not CheckTemplateFile(templatePath) then echo "index.html"&err_16
	...
	with templateObj 
		...
		.indexpath		
		.parseCommon		
		createTextFile  ClearHtml(.content), sitePath&setting.LanguagePath&"wap/index"&FileExt,""
	end with	
	set templateobj =nothing
	
	if action<>"allwap" and action<>"daywap" then alertMsgAndGo "生成首页成功","AspCms_MakeHtml.asp?actType="&actType		
End Sub
...
Sub makeAllWapContent
	...
	if action<>"allwap" then alertMsgAndGo "生成内容页成功","AspCms_MakeHtml.asp?actType="&actType	
End Sub
...
'按分类生成Wap
Sub makeWapContentBySortID
	dim rs,sql,SortID
	wap="wap"
	twap=wap
	ewap=wap&"/"
	swap="/"&wap
	SortID=getForm("wcsortid","post")
	if SortID=0 then alertMsgAndGo "请选择栏目","MakeHtml.asp?actType="&actType	
	sql="select * from {prefix}Content where ContentStatus=1 and GroupID<3 and SortID in("&getSubSort(SortID, 1)&") and LanguageID="&cint(session("languageID"))
	set rs=conn.Exec(sql,"r1")	
	Do While not rs.Eof		
	...
	Loop
	rs.close : set rs =nothing	
	if action<>"allwap" then alertMsgAndGo "生成指定内容页成功","AspCms_MakeHtml.asp?actType="&actType	
End Sub
...
Sub makeAllWapAbout
	...
	ewap=wap&"/"
	swap="/"&wap
	sql = "select SortID,SortType from {prefix}Sort where WapStatus=1 and SortType=1 and GroupID<3 and LanguageID="&cint(session("languageID"))
	set rs=conn.exec(sql,"r1")
	do while not rs.Eof
		makeAbout rs("SortID"), 0, 1
		rs.movenext
	loop
	rs.close : set rs =nothing	
	if action<>"allwap" then alertMsgAndGo "生成所有单页成功","AspCms_MakeHtml.asp?actType="&actType
End Sub
...
Sub delAllWapHtml
	dim styles,style
	styles=split("news,down,pic,product,about,",",")	
	if isExistFile(sitePath&swap&"/index.html") then delFile sitePath&swap&"/index.html"	'删除首页	
	for each style in styles
		if style="news" or style="down" or style="pic" or style="product" then DelWaphtml(style&"list")	'删除列表页
		DelWaphtml(style)			'删除详细页				
	next	
	'删除指定的生成目录
	styles=""
	styles=conn.exec("select SortFolder from {prefix}Sort where not isnull(SortFolder)","arr")
	for each style in styles
		'if not isnul(style) then echo "/"&sitePath&style&"<br>"
		'Delhtml(style)			'删除详细页			
		if not isnul(style) and isExistFolder("/"&sitePath&"/"&ewap&style) then delFolder("/"&sitePath&"/"&ewap&style)	
	next
End Sub
...
Sub delWaphtml(str)
	dim fileListArray,fileAttr,i
	fileListArray= getFileList("/"&sitePath&"/"&ewap&str)
	if instr(fileListArray(0),",")>0 then		
		for  i = 0 to ubound(fileListArray)
			fileAttr=split(fileListArray(i),",")	
			if GetExtend(fileAttr(0))=replace(FileExt,".","") then delFile fileAttr(4)	
